...
# def am_softmax_loss(logits, target, s=30.0, m=0.35):
#     logits = logits.clone()
#     logits[torch.arange(logits.size(0)), target] -= m          # add margin
#     return F.cross_entropy(logits * s, target)                 # scale and CE
...
# def _log_mel(x: torch.Tensor, mel):
#     return torch.log(mel(x.unsqueeze(1)).clamp_min(1e-5))

# def mel_loss(out_waveform: torch.Tensor, reconstructed_target: torch.Tensor, mel):
#     return F.l1_loss(_log_mel(out_waveform, mel), _log_mel(reconstructed_target, mel))
...
class MultiScaleMelLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        for mel_fn in self.transforms:
            loss += F.l1_loss(
                self._log_mel(pred, mel_fn),
                self._log_mel(target, mel_fn)
            )
    ...
    for batch in tqdm(loader):
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                if cfg.use_descript:
                    # x = model.quantizer.preprocess(clean_audio, cfg.sample_rate)
...
def train_epoch(
    ...
    for batch in tqdm(loader):
        ...
        if cfg.dynamic_mixing:
            ...
            ]).max(dim=0)[0].clamp_min(1e-8)

            # Apply normalization
            mixed = mixed / max_amp
            # s1 = s1 / max_amp
            # s2 = s2_scaled / max_amp
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            if not O:
                ...
                torchaudio.save('sample.wav', out_waveform[0].float().cpu().detach().unsqueeze(0), sample_rate=16000)
                torchaudio.save('sample_rec.wav', reconstructed_target[0].float().cpu().detach().unsqueeze(0), sample_rate=16000)
                torchaudio.save('sample_tgt.wav', clean_audio[0].cpu().float(), sample_rate=16000)
                O = True
            ...
            ce_speaker_loss = 0
            # ce_speaker_loss = F.cross_entropy(speaker_label_pred, speaker_label)

            _mel_loss = 0
            if cfg.add_mel_loss:
                if cfg.use_descript:
                    _mel_loss = mel_loss(out_waveform, reconstructed_target.squeeze()).mean()
                else:
                    _mel_loss = F.l1_loss(_out.transpose(1,2), model.get_latent(clean_audio)).mean()
            ...
            if cfg.add_mel_loss:
                writer.add_scalar(
                    f"latent_loss/train", _mel_loss.detach().cpu(), step
                )
            ...
            loss = si_sdr_loss.mean()*w1 + _mel_loss*w2 + l1_latent_loss*w3 + ce_speaker_loss * w4
            loss.backward()

            writer.add_scalar(
                f"Total_loss/train", loss.detach().cpu(), step
            )
...
def train(
    ...
    **kwargs,
    ...
    #     sample_rate=cfg.sample_rate, n_fft=1024, hop_length=160, n_mels=80, f_max=16000
    # ).to(device)
